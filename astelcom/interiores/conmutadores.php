...
<?php
mysql_select_db($database_con_admin, $con_admin);
$qry33 = "SELECT titulo FROM categoria WHERE id_ca='$id_ca'";
$r33 = mysql_query($qry33, $con_admin) or die(mysql_error());
if ( $r33 !== false && mysql_num_rows($r33) > 0 ) 
{
	$a33 = mysql_fetch_assoc($r33);
	$title33 = stripslashes($a33['titulo']);	
}
?>	
<h2><?php echo $title33;?></h2>
</div>
<!------------------------------->
...
<?php              
mysql_select_db($database_con_admin, $con_admin);
$qry2 = "SELECT * FROM productos WHERE id_ca='$id_ca' ORDER BY orden ASC";
...
if ( $r2 !== false && mysql_num_rows($r2) > 0 ) 
{
	while ($a2 = mysql_fetch_assoc($r2))
	{
		...
		$descripcion = stripslashes($a2['descripcion']);
		$ids = stripslashes($a2['id_p']); 
		$id_ca = stripslashes($a2['id_ca']);  
?>		
       <div id="rectangulo1">
        	<div id="imagen_conmuta">
            
       	    <img src="../img/productos/<?php echo $ima;?>" width="200" height="175" />
