...
$qry20 = "SELECT * FROM productos WHERE id_ca='$id_ca'";
$r20 = mysql_query($qry20, $con_admin) or die(mysql_error());
if ( $r20 !== false && mysql_num_rows($r20) > 0 ) 
{
	$a20 = mysql_fetch_assoc($r20);
	$id_c0= stripslashes($a20['id_ca']);
}
$menu=$_GET['menu'];
?>
...
<div id="caja">
	<p class="textoboldnaranja" name="titulo_seccion">Productos</p>&nbsp;
...
<?php
mysql_select_db($database_con_admin, $con_admin);
$query = "SELECT COUNT(id_p) FROM productos WHERE id_ca='$id_ca'";
...
?>  
                <div class="nuevo">
                    <a href="producto_nuevo.php?menu=galeria.php&id_ca=<?php echo $id_ca;?>"><font color="#000000">Nuevo Producto +</font></a>
                </div>
<div id="caja">
	<p id="titulo_orden">
...
<?php              
mysql_select_db($database_con_admin, $con_admin);
$qry2 = "SELECT * FROM productos WHERE id_ca=$id_ca ORDER BY orden ASC $pages2->limit";
...
<div id="boton_1">
	<button class="boton-formulario" onclick="location.href='producto_editar.php?id_p=<?php echo $ids; ?>&id_ca=<?php echo $id_ca;?>'">Actualizar</button>
</div>
<div id="boton_2">
	<button class="boton-formulario2" onclick="if (confirm('¿Desea eliminar el producto: <?php echo $ima; ?>?')==true){window.location.href='producto_eliminar.php?id_p=<?php echo $ids; ?>&menu=geleria.php'}">Eliminar</button>
</div>
<?php
	}
}
