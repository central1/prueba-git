...
<div id="caja">
	<p class="textoboldnaranja" id="titulo_seccion" name="titulo_seccion">Productos</p>
...
<div id="caja_tamaños">
    ...
    <span class="pie-naranja"><strong>Tamaño de Imágen: </strong></span>200 x 175 px</p>
</div>
<div id="caja_archivos">
...
<form id="form2" name="form2" method="post" action="productos.php?menu=galeria.php&id_ca=<?php echo $id_ca;?>">
<p id="boton_2">
...
if (isset($_POST['guardar']))
{
	...
	if ($nombre_archivo4!="")
	{ 
		if ( strpos($tipo_archivo4 , "png") ||  strpos($tipo_archivo4 , "ico") || strpos($tipo_archivo4 , "gif") ||(strpos($tipo_archivo4 , "jpeg"))) 
		{
			copy($_FILES['imagen']['tmp_name'], $carpeta.$nombre_archivo4);
			extract($_REQUEST);
			mysql_select_db($database_con_admin, $con_admin);
			$qry44 = "INSERT INTO productos (imagen,orden,titulo,id_ca,descripcion) VALUES ('$nombre_archivo4','$orden','$titulo','$id_ca','$descripcion')";
			mysql_query($qry44, $con_admin) or die(mysql_error());
			echo "<SCRIPT LANGUAJE='javascript'>window.location='productos.php?menu=galeria&id_ca=$id_ca' </SCRIPT>";	
		}
		else
		{  
			echo "<script languaje='javascript'>alert('ERROR EN LA IMAGEN')</script>";
			echo"<script language='javascript'>window.location='producto_nuevo.php&id_ca=$id_ca'</script>";	
		}	
	}
	else
	{
